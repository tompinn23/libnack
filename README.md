...
the filename, so an embedded tileset needs no hint. The decoders are LodePNG
and TurboJPEG, vendored under `third_party/` and built from source, so there
is still no image library to install. Decoding an image is parsing untrusted
input, and that is the last place to prefer something hand-written over code
that has been attacked for years. See `third_party/VENDORING.md` for what was
taken and how to update it.
...
`tests/image_test.c` covers the seam between libnack and the vendored
decoders. PNGs are generated by `tools/mkpng.py` across every colour type, bit
depth and row filter, interlaced and not, and compared pixel for pixel against
reference data the decoder had no part in producing. JPEG has nothing exact to
compare against, so a known image is compressed and decoded back and checked
for size, orientation, opacity and how far the values drifted. Truncated and
corrupt input of both kinds must be refused rather than crash.
...
Vendored under `third_party/`: LodePNG (zlib licence) and libjpeg-turbo (IJG,
with BSD-3-Clause and zlib for parts). Both are permissive and neither is
copyleft, so linking libnack carries no obligation beyond keeping the notices.
`third_party/VENDORING.md` records the versions and what was taken.
