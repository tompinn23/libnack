...
nack_result nack_get_error(const char **message)
{
    if (message)
...
double nack_time_seconds(void)
{
    return (double)nack_time_ns() / 1e9;
}
...
{
    size_t next = (nack__g.queue_tail + 1u) % NACK_EVENT_QUEUE_CAP;
    if (next == nack__g.queue_head) {
        ...
        nack__g.queue_head = (nack__g.queue_head + 1u) % NACK_EVENT_QUEUE_CAP;
    }
...
{
    static nack_event scratch;
    memset(&scratch, 0, sizeof scratch);
    scratch.type = type;
    scratch.window = w;
    scratch.time_ns = nack_time_ns();
    return &scratch;
}

static bool nack__pop(nack_event *out)
{
...
{
    nack_event *ev = nack__event_begin(type, w);
    nack__push_event(ev);
}
...
void nack__emit_resize(nack_window *w, int width, int height, int fb_width, int fb_height)
{
    ...
    ev->size.fb_width = fb_width;
    ev->size.fb_height = fb_height;
    nack__push_event(ev);
}
...
void nack__emit_scale(nack_window *w, float scale)
{
    if (scale <= 0.0f || w->scale == scale)
        return;
    w->scale = scale;
    nack_event *ev = nack__event_begin(NACK_EVENT_WINDOW_SCALE, w);
    ...
    if (!focused) {
        ...
        for (int i = 0; i < NACK_KEY_COUNT; ++i) {
            if (nack__g.keys[i]) {
                ...
                nack__emit_key(w, (nack_key)i, 0, nack__g.mods, false, false);
            }
        }
        nack__g.mods = 0;
    }
    nack__emit_simple(w, focused ? NACK_EVENT_WINDOW_FOCUS : NACK_EVENT_WINDOW_BLUR);
}
                    ...
                    bool down, bool repeat)
{
    ...
    nack_event *ev = nack__event_begin(down ? NACK_EVENT_KEY_DOWN : NACK_EVENT_KEY_UP, w);
    ev->key.key = key;
...
void nack__emit_text(nack_window *w, const char *utf8)
{
    if (!utf8 || !utf8[0])
        return;
    nack_event *ev = nack__event_begin(NACK_EVENT_TEXT, w);
    size_t n = strlen(utf8);
    if (n >= sizeof ev->text.utf8)
        n = sizeof ev->text.utf8 - 1;
    memcpy(ev->text.utf8, utf8, n);
    ev->text.utf8[n] = '\0';
    nack__push_event(ev);
}
...
void nack__emit_mouse_move(nack_window *w, double x, double y, uint32_t mods)
{
    ...
    ev->motion.dx = x - w->mouse_x;
    ev->motion.dy = y - w->mouse_y;
    ev->motion.x = x;
    ev->motion.y = y;
    ev->motion.mods = mods;
    w->mouse_x = x;
                             ...
                             uint32_t mods)
{
    ...
    nack_event *ev = nack__event_begin(down ? NACK_EVENT_MOUSE_DOWN : NACK_EVENT_MOUSE_UP, w);
    ev->button.button = button;
    ev->button.x = x;
    ev->button.y = y;
    ev->button.mods = mods;
    ev->button.click_count = clicks;
    nack__push_event(ev);
}
...
void nack__emit_scroll(nack_window *w, double dx, double dy, uint32_t mods, bool precise)
{
    if (dx == 0.0 && dy == 0.0)
        return;
    nack_event *ev = nack__event_begin(NACK_EVENT_MOUSE_SCROLL, w);
    ev->scroll.dx = dx;
    ev->scroll.dy = dy;
    ev->scroll.mods = mods;
    ev->scroll.precise = precise;
    nack__push_event(ev);
}
...
static void nack__purge_window_events(nack_window *w)
{
    ...
    while (read != nack__g.queue_tail) {
        if (nack__g.queue[read].window != w) {
            ...
            write = (write + 1u) % NACK_EVENT_QUEUE_CAP;
        }
        read = (read + 1u) % NACK_EVENT_QUEUE_CAP;
    }
...
const char *nack_backend_name(nack_backend backend)
{
    switch (backend) {
...
static const nack_backend_vt *nack__select_backend(nack_backend preferred)
{
#if defined(NACK_PLATFORM_WIN32)
    (void)preferred;
...
static bool nack__try_init_backends(const nack_init_desc *desc)
{
#if defined(NACK_PLATFORM_UNIX)
    const nack_backend_vt *order[2] = { NULL, NULL };
    size_t n = 0;
    const nack_backend_vt *first = nack__select_backend(desc->backend);
    if (!first)
...
#else
    const nack_backend_vt *vt = nack__select_backend(desc->backend);
    if (!vt)
        return nack__fail(NACK_ERROR_NO_BACKEND, "no display backend compiled in");
    nack__g.vt = vt;
    if (vt->init(desc))
...
bool nack_init(const nack_init_desc *desc)
{
    if (nack__g.initialized)
        return true;

    nack_init_desc local;
...
bool nack_is_initialized(void)
{
    return nack__g.initialized;
}

nack_backend nack_get_backend(void)
{
...
void nack_window_desc_defaults(nack_window_desc *desc)
{
    if (!desc)
        return;
...
void nack_gl_desc_defaults(nack_gl_desc *desc)
{
    if (!desc)
        return;
    memset(desc, 0, sizeof *desc);
    desc->major = 3;
    desc->minor = 3;
    desc->profile = NACK_GL_PROFILE_CORE;
}

void nack_framebuffer_desc_defaults(nack_framebuffer_desc *desc)
{
    if (!desc)
        return;
    memset(desc, 0, sizeof *desc);
...
#define NACK_REQUIRE_INIT_RET(ret)                                            \
    do {                                                                      \
        if (!nack__g.initialized) {                                           \
            nack__fail(NACK_ERROR_NOT_INITIALIZED, "nack_init() not called"); \
            return ret;                                                       \
        }                                                                     \
    } while (0)
...
nack_window *nack_window_create(const nack_window_desc *desc)
{
    ...
    nack_window *w = (nack_window *)nack__calloc(1, sizeof *w);
    if (!w)
        return NULL;
...
void nack_window_destroy(nack_window *window)
{
...
void nack_window_show(nack_window *window)
{
    if (!window) return;
...
void nack_window_hide(nack_window *window)
{
    if (!window) return;
...
void nack_window_set_size(nack_window *window, int width, int height)
{
...
void nack_window_get_size(const nack_window *window, int *width, int *height)
{
    if (width)  *width  = window ? window->width : 0;
    if (height) *height = window ? window->height : 0;
}

void nack_window_get_framebuffer_size(const nack_window *window, int *width, int *height)
{
    if (width)  *width  = window ? window->fb_width : 0;
    if (height) *height = window ? window->fb_height : 0;
}

void nack_window_get_position(const nack_window *window, int *x, int *y)
{
    if (x) *x = window ? window->pos_x : 0;
    if (y) *y = window ? window->pos_y : 0;
}

void nack_window_set_position(nack_window *window, int x, int y)
{
    if (window && window->vt->window_set_position)
        window->vt->window_set_position(window, x, y);
}

float nack_window_get_content_scale(const nack_window *window)
{
    return window ? window->scale : 1.0f;
}

void nack_window_set_size_limits(nack_window *window, int min_width, int min_height,
...
void nack_window_set_size_increments(nack_window *window, int dw, int dh)
{
    if (!window) return;
...
void nack_window_set_fullscreen(nack_window *window, bool fullscreen)
{
    if (window && window->vt->window_set_fullscreen)
        window->vt->window_set_fullscreen(window, fullscreen);
}

bool nack_window_is_fullscreen(const nack_window *window)  { return window && window->fullscreen; }
bool nack_window_is_focused(const nack_window *window)     { return window && window->focused; }
bool nack_window_is_minimized(const nack_window *window)   { return window && window->minimized; }
bool nack_window_is_maximized(const nack_window *window)   { return window && window->maximized; }
bool nack_window_should_close(const nack_window *window)   { return window && window->should_close; }

void nack_window_set_should_close(nack_window *window, bool value)
{
    if (window)
        window->should_close = value;
}

void *nack_window_get_user_data(const nack_window *window)
{
    return window ? window->user_data : NULL;
}

void nack_window_set_user_data(nack_window *window, void *user_data)
{
    if (window)
        window->user_data = user_data;
}

void nack_window_set_cursor_shape(nack_window *window, nack_cursor_shape shape)
{
...
void nack_window_set_cursor_mode(nack_window *window, nack_cursor_mode mode)
{
    if (!window)
        return;
...
nack_cursor_mode nack_window_get_cursor_mode(const nack_window *window)
{
    return window ? window->cursor_mode : NACK_CURSOR_MODE_NORMAL;
}

void nack_window_request_redraw(nack_window *window)
{
    if (!window)
        return;
...
void nack_window_get_native(const nack_window *window, nack_native_window *out)
{
    if (!out)
        return;
...
bool nack_poll_event(nack_event *event)
{
...
bool nack_wait_event(nack_event *event)
{
    return nack_wait_event_timeout(event, -1.0);
}

bool nack_wait_event_timeout(nack_event *event, double timeout)
{
    NACK_REQUIRE_INIT_RET(false);
    if (!event)
        return false;
    ...
    double deadline = nack_time_seconds() + timeout;
    for (;;) {
...
bool nack_key_is_down(nack_key key)
{
    if (key <= 0 || key >= NACK_KEY_COUNT)
        return false;
...
uint32_t nack_get_mods(void)
{
    return nack__g.mods;
}

bool nack_mouse_button_is_down(int button)
{
    if (button < 0 || button >= NACK_MOUSE_BUTTON_COUNT)
        return false;
...
void nack_get_mouse_position(nack_window *window, double *x, double *y)
{
...
nack_gl_context *nack_gl_context_create(nack_window *window, const nack_gl_desc *desc)
{
    NACK_REQUIRE_INIT_RET(NULL);
    if (!window) {
        nack__fail(NACK_ERROR_INVALID_ARGUMENT, "nack_gl_context_create: null window");
        return NULL;
    }
    nack_gl_desc d;
    nack_gl_desc_defaults(&d);
    if (desc)
        d = *desc;
...
void nack_gl_context_destroy(nack_gl_context *context)
{
    if (!context)
        return;
...
bool nack_gl_make_current(nack_window *window, nack_gl_context *context)
{
    NACK_REQUIRE_INIT_RET(false);
...
void nack_gl_swap_buffers(nack_window *window)
{
    if (window && nack__g.vt && nack__g.vt->gl_swap_buffers)
        nack__g.vt->gl_swap_buffers(window);
}

void nack_gl_set_swap_interval(int interval)
{
    if (!nack__g.initialized)
        return;
...
void *nack_gl_get_proc_address(const char *name)
{
...
bool nack_clipboard_set_text(const char *utf8)
{
    NACK_REQUIRE_INIT_RET(false);
...
const char *nack_clipboard_get_text(void)
{
...
bool nack_primary_set_text(const char *utf8)
{
...
const char *nack_primary_get_text(void)
{
